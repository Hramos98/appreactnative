    ...
    <View style={styles.viewBody}>
      ...
      {restaurants ? (
        ...
        <View style={styles.loaderRestaurants}>
          ...
          <Text>Cargando Articulos</Text>
        </View>
      )}
      <Toast ref={toastRef} position="center" opacity={1} />
      <Loading text="Eliminando Articulo" isVisible={isVisibleLoding} />
    </View>
  );
}
...
function Restaurant(props) {
  ...
  const confirmRemoveFavorite = () => {
    Alert.alert(
      "Eliminar Articulo de Favoritos",
      "¿Estas seguro de que quieres eliminar el Articulo de favoritos?",
      [
        {
          text: "Cancel",
          style: "cancel"
    ...
    db.collection("favorites")
      ...
      .then(response => {
        response.forEach(doc => {
          ...
          db.collection("favorites","productos","reviews")
            .doc(idFavorite)
            .delete()
            .then(() => {
              setIsVisibleLoading(false);
              setReloadRestaurants(true);
              toastRef.current.show("Articulo eliminado correctamente");
            })
            .catch(() => {
              toastRef.current.show(
                "Error al eliminar el Articulo , intentelo más tarde"
              );
            });
        });
      });
